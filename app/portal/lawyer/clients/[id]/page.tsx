...
// IDENTICAL to /portal/fp/clients/[id]/page.tsx — per the brief, the lawyer
...
export default function LawyerClientDetailPage({ params }: { params: { id: string } }) {
  const [client, setClient] = useState<LawyerClientDetail | null>(null)
  ...
  const loadClient = useCallback(() => {
    setLoading(true)
    setError('')
    fetch(`/api/portal/lawyer/clients/${params.id}`)
      .then(r => r.json())
      .then(data => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    setMessageError('')
    try {
      const res = await fetch('/api/portal/lawyer/message', {
        method: 'POST',
            ...
            {client.messages.length === 0 && !messageSent ? (
              ...
              <div className="space-y-3 mb-4">
                {client.messages.map((msg) => {
                  const isFromLawyer = msg.createdBy !== 'Michael Fox'
                  return (
                    <div
                      key={msg.id}
                      className={`flex ${isFromLawyer ? 'justify-end' : 'justify-start'}`}
                    >
                      <div
                        className={`max-w-sm px-4 py-3 rounded-xl text-sm font-body ${
                          isFromLawyer ? 'bg-navy text-white' : 'bg-gray-100 text-navy'
                        }`}
                      >
