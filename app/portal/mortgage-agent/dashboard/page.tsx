...
const quickActions = [
  { title: 'Add Referral', description: 'Submit a new client referral', icon: UserPlus, href: '/portal/mortgage-agent/add-referral' },
  { title: 'View Clients', description: 'See all your referred clients', icon: Users, href: '/portal/mortgage-agent/clients' },
  { title: 'Message Michael', description: 'Send a message or ask a question', icon: MessageSquare, href: '/portal/mortgage-agent/messages' },
]

// ─── Page ─────────────────────────────────────────────────────────────────────

export default function MortgageAgentDashboardPage() {
  const { user } = useUser()
  // Greets the impersonated partner by name when an admin is viewing as them,
  // otherwise the signed-in mortgage agent's own first name.
  ...
  useEffect(() => {
    fetch('/api/portal/mortgage-agent/dashboard')
      .then(r => r.json())
      .then(data => {
        // Payload shape: { stats, recent, warning? } — always set, even on warning.
        if (data?.stats) setStats(data.stats)
        if (Array.isArray(data?.recent)) setRecent(data.recent)
        if (data?.warning) console.warn('[mortgage-agent/dashboard] warning:', data.warning)
      })
      .catch(err => console.error('[mortgage-agent/dashboard] fetch error:', err))
  ...
  return (
    <div>
      ...
      <div className="bg-lime/10 border border-lime/30 rounded-xl p-4 mb-6 flex items-center justify-between">
        ...
        <Link
          href="/portal/mortgage-agent/clients"
                ...
                return (
                  ...
                  >
                    <td className="px-5 py-3">
                      <Link
                        href={`/portal/mortgage-agent/clients/${row.dealId}`}
