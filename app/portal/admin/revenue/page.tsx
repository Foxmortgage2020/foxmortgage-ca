...
function ActualChip() {
  return (
    <span className="inline-flex items-center text-[10px] font-semibold uppercase tracking-wide px-1.5 py-0.5 rounded bg-cool-100 text-cool-700 border border-cool-200 align-middle">
      actual
    </span>
  )
}
    ...
    <div className="space-y-5">
      ...
      >
        ...
        {years.length > 0 ? (
          ...
          <p className="text-sm text-cool-400 font-ui">No funded history to chart.</p>
        )}
      </SectionCard>
      ...
      >
        {book ? (
          ...
          <p className="text-sm text-cool-400 font-ui">The renewal read is unavailable right now.</p>
        )}
      </SectionCard>
      ...
      <SectionCard title="Referral partners">
        ...
        <p className="text-[11px] text-amber-800 bg-amber-50 border border-amber-200 rounded-lg px-2.5 py-1.5 font-ui mt-3">
        ...
        <div className="mt-4">
          ...
          {recentReferrals.length === 0 ? (
            <p className="text-xs text-cool-400 font-ui">No attributed referrals to show.</p>
          ) : (
            <div className="space-y-1">
              {recentReferrals.slice(0, 6).map(r => (
                <div key={r.dealId} className="flex items-center justify-between gap-3 text-xs font-ui border-t border-cool-100 py-1">
                  <span className="text-navy truncate">{r.borrower}</span>
                  <span className="flex items-center gap-2 shrink-0 text-cool-500">
                    <span>{r.partner ?? 'unattributed'}</span>
                    <span className="text-cool-400">{r.stage}</span>
                  </span>
                </div>
              ))}
            </div>
      ...
      <SectionCard title={`Goal pacing ${year}`}>
        ...
        {unmappedStages.length > 0 && (
          <div className="mb-4 rounded-lg border border-amber-300 bg-amber-50 px-3 py-2">
            ...
            <p className="text-xs text-amber-800 font-ui mt-0.5">
              {unmappedStages
          ...
          <div className="overflow-x-auto">
            <div className="min-w-[520px] space-y-1">
              {pipeline.stale.map(d => (
                <div
                  key={d.id}
                  className="flex items-center gap-2 text-xs font-ui border-t border-cool-100 py-1.5"
                >
                  <a
                    href={zohoDealUrl(d.id)}
                    target="_blank"
                    rel="noreferrer"
                    className="flex-1 truncate text-navy hover:text-ink"
                  >
                    {d.dealName}
                  </a>
                  <span className="w-40 text-cool-400 truncate hidden sm:block">{d.stage}</span>
                  <span className="w-24 text-right text-cool-500">
                  ...
                  <span className="w-14 text-right text-cool-500">{fmtMoneyCompact(d.amount)}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {mixes.map(mix =>
            mix.renders ? (
                ...
                <div className="space-y-1">
                  {mix.rows.map(r => (
                    <div key={r.key} className="flex items-center gap-2">
                      <div className="w-28 text-xs font-ui text-cool-600 truncate">{r.key}</div>
                      <div className="flex-1 bg-cool-100 rounded h-3">
                        <div
                          className="bg-navy h-3 rounded"
                          style={{ width: `${(r.count / Math.max(1, mix.coveredCount)) * 100}%` }}
                        />
                      </div>
                      <div className="text-xs font-ui text-cool-500 w-8 text-right">{r.count}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div key={mix.label}>
                ...
                <p className="text-xs text-cool-400 font-ui">
      ...
      <SectionCard title="Conversion funnel">
        <div className="space-y-1.5 mb-3">
          {[...pipeline.ordered, ...pipeline.other].map(row => {
            ...
            return (
              <div key={row.stage} className="flex items-center gap-2">
                <div className="w-44 text-xs font-ui text-cool-700 truncate">{row.stage}</div>
                <div className="flex-1 bg-cool-100 rounded h-4">
                  <div className="bg-navy/70 h-4 rounded" style={{ width: `${(row.volume / maxVol) * 100}%` }} />
                </div>
                <div className="text-xs font-ui text-cool-600 w-24 text-right">
                  {row.count} · {fmtMoneyCompact(row.volume)}
                </div>
                <div className="text-[11px] font-ui w-16 text-right">
                  {weight != null ? (
                    <span className="text-cool-400">{`w ${weight}`}</span>
                  ) : (
                    <span
          ...
          <div className="flex items-center gap-2 pt-1 border-t border-cool-100">
            <div className="w-44 text-xs font-ui font-semibold text-navy">Funded, trailing 12</div>
            <div className="flex-1" />
            <div className="text-xs font-ui font-semibold text-navy w-40 text-right">
        ...
        <div className="mt-4">
          ...
          {leadStats ? (
            <div className="space-y-1">
              {leadStats.rows.map(r => (
                <div key={r.source} className="flex items-center gap-2">
                  <div className="w-44 text-xs font-ui text-cool-600 truncate">{r.source}</div>
                  <div className="flex-1 bg-cool-100 rounded h-3">
                    <div
                      className="bg-navy h-3 rounded"
              ...
              <p className="text-[11px] text-cool-400 font-ui pt-1">
            ...
            <p className="text-xs text-cool-400 font-ui">The Zoho leads read failed; reload for the source split.</p>
          )}
        </div>
      </SectionCard>
        ...
        {pnl.state === 'ok' ? (
          <div className="overflow-x-auto">
            <table className="w-full text-xs font-ui">
              <thead>
                <tr className="text-left text-cool-400">
              ...
              <tbody>
                {Array.from(new Set(pnl.months.flatMap(m => m.classes.map(c => c.name)))).map(name => (
                  <tr key={name} className="border-t border-cool-100">
                    <td className="py-1 pr-3 text-cool-700">{name}</td>
                    {pnl.months.map(m => {
          ...
          <p className="text-sm text-amber-800 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 font-ui">
            {pnl.message} The tile renders nothing rather than a stale or partial P&L.
          </p>
        ) : (
          <div>
            <p className="text-sm text-cool-600 font-ui mb-2">
            ...
            <ul className="text-xs text-cool-500 font-ui list-disc pl-5 space-y-1">
...
function FragmentCells({ revenue, expenses, net }: { revenue: number; expenses: number; net: number }) {
  return (
    <>
      <td className="py-1 pr-2 text-right text-cool-600">{fmtMoneyCompact(revenue)}</td>
      <td className="py-1 pr-2 text-right text-cool-600">{fmtMoneyCompact(expenses)}</td>
...
function CompRowLine({
  ...
  return (
    <div className="flex items-baseline gap-2 flex-wrap">
      <span className="text-xs font-ui text-cool-700 w-56">{label}</span>
      <span className="text-xs font-ui font-semibold text-navy">{value}</span>
      {!confirmed && (
      ...
      {note && <span className="text-[11px] font-ui text-cool-400">{note}</span>}
    </div>
  )
}
...
function Stat({
  ...
  return (
    <div className="border border-cool-100 rounded-lg px-3 py-2 bg-cool-50/50">
      <p className="text-[11px] font-ui text-cool-400 uppercase tracking-wide">{label}</p>
      <p
      ...
      {sub != null && <p className="text-[11px] font-ui text-cool-500 mt-0.5">{sub}</p>}
    </div>
  )
}
...
function MonthBars({
  ...
  return (
    <div>
      <p className="text-[11px] font-ui text-cool-400 mb-1">{valueLabel}</p>
      <div className="flex items-end gap-1 h-28 overflow-x-auto" data-testid="month-bars">
        {rows.map((r, i) => (
          ...
          >
            ...
            <div
              className={`w-full rounded-t ${r.value > 0 ? (r.cls ?? barClass) : 'bg-cool-200'}`}
              style={{ height: `${Math.max(2, (r.value / max) * 80)}%` }}
            />
            <span className="text-[10px] font-ui text-cool-400 mt-1 leading-none">
