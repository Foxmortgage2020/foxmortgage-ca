...
const BACKLOG: { title: string; note: string }[] = [
  { title: 'Command Centre Phase B: page interiors (B1 + B2a shipped 2026-07-16)', note: 'The 2026-07-14 shell redesign (Phase A) shipped the grouped sidebar, the Desk strip, decision badges, and the lime-as-decisions rule. B1 (the lifecycle spine) shipped ONE canonical lifecycle definition (config/lifecycle.ts): the board groups its seven columns under Intake / Underwriting / Fulfilment / Complete & paid, the Home pipeline table groups by the same phases, and every deal room carries a journey stepper with the BRX deck steps per deal shape. B2a (stage truth) made Zoho the position source: every board card sits where its Zoho display stage says (five of seven live files moved to their true columns), every portal stage read normalizes to display space, and the Ready To Close weight amber cleared with zero weight edits. Next: B2b (the Deals redesign on truthful positions) and B3 (agent mode). The 72-file visual debt inventory in docs/lifecycle-b1-2026-07-16.md feeds them. A persisted backfill-scan result would also light the Desk strip\u2019s manual-match fragment.' },
