...
const quickActions = [
  { title: 'Add Referral', description: 'Submit a new client referral', icon: UserPlus, href: '/portal/realtor/add-referral' },
  { title: 'View Clients', description: 'See all your referred clients', icon: Users, href: '/portal/realtor/clients' },
  { title: 'Message Michael', description: 'Send a message or ask a question', icon: MessageSquare, href: '/portal/realtor/messages' },
]
...
export default function RealtorDashboardPage() {
  ...
  useEffect(() => {
    fetch('/api/portal/realtor/dashboard')
      .then(r => r.json())
      .then(data => {
        // Payload shape: { stats, recent, warning? } — always set, even on warning.
        if (data?.stats) setStats(data.stats)
        if (Array.isArray(data?.recent)) setRecent(data.recent)
        if (data?.warning) console.warn('[realtor/dashboard] warning:', data.warning)
      })
      .catch(err => console.error('[realtor/dashboard] fetch error:', err))
                ...
                return (
                  ...
                  >
                    <td className="px-5 py-3">
                      <Link
                        href={`/portal/realtor/clients/${row.dealId}`}
