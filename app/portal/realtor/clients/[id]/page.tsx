...
// IDENTICAL to /portal/fp/clients/[id]/page.tsx — per the brief, the realtor
// tracker mirrors the FP one verbatim.
...
export default function RealtorClientDetailPage({ params }: { params: { id: string } }) {
  const [client, setClient] = useState<RealtorClientDetail | null>(null)
  ...
  const loadClient = useCallback(() => {
    setLoading(true)
    setError('')
    fetch(`/api/portal/realtor/clients/${params.id}`)
      .then(r => r.json())
      .then(data => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    setMessageError('')
    try {
      const res = await fetch('/api/portal/realtor/message', {
        method: 'POST',
            ...
            {client.messages.length === 0 && !messageSent ? (
              ...
              <div className="space-y-3 mb-4">
                {client.messages.map((msg) => {
                  const isFromRealtor = msg.createdBy !== 'Michael Fox'
                  return (
                    <div
                      key={msg.id}
                      className={`flex ${isFromRealtor ? 'justify-end' : 'justify-start'}`}
                    >
                      <div
                        className={`max-w-sm px-4 py-3 rounded-xl text-sm font-body ${
                          isFromRealtor ? 'bg-navy text-white' : 'bg-gray-100 text-navy'
                        }`}
                      >
