...
export async function POST(req: NextRequest) {
  try {
    ...
    // be able to post messages that land as if the lawyer sent them.
    ...
    const webhookUrl = process.env.LAWYER_MESSAGE_WEBHOOK_URL
    if (webhookUrl) {
      ...
      if (!webhookRes.ok) {
        console.error('[Lawyer Message] n8n webhook returned', webhookRes.status)
      }
    } else {
      // Webhook not yet configured — log for now
      console.log('[Lawyer Message] Webhook URL not set. Message data:', {
        lawyerName: fullName,
        lawyerEmail: ctx.actor.email,
        messageBody,
        context: context || 'general',
    ...
    console.error('[Lawyer Message Error]', error)
