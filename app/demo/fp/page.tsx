// ─── Demo FP dashboard ─────────────────────────────────────────────────────────
// Mirrors app/portal/fp/dashboard/page.tsx presentation, fed entirely by inline
// static data (app/demo/fp/_data/demo-data.ts). ZERO fetch, no Clerk, read-only.
// All internal links point to /demo/fp/* and "Add Referral" is a disabled no-op.
...
// ─── Helpers (copied from the live dashboard) ─────────────────────────────────
...
// "Add Referral" is intentionally a disabled no-op in the read-only demo.
const quickActions = [
  { title: 'Add Referral', description: 'Submit a new client referral', icon: UserPlus, href: null, disabled: true },
  { title: 'View Clients', description: 'See all your referred clients', icon: Users, href: '/demo/fp/clients', disabled: false },
  { title: 'Message Michael', description: 'Send a message or ask a question', icon: MessageSquare, href: '/demo/fp/messages', disabled: false },
]

export default function DemoFPDashboardPage() {
  const stats: DemoDashboardStats = DEMO_DASHBOARD_STATS
  const recent = DEMO_RECENT
  const firstName = 'Sample Planner'
  ...
  return (
    <div>
      ...
      {/* Strategic Mortgage Monitoring — observed analytical finding (not a guarantee) */}
      <div className="bg-navy text-white rounded-xl p-6 mb-8">
        <h3 className="font-heading font-bold text-lg mb-2">Strategic Mortgage Monitoring</h3>
        <p className="font-body text-sm text-gray-300 leading-relaxed">
          Every mortgage your clients refer is checked against the market every day. In Fox
          Mortgage&apos;s experience to date, about 20% of monitored clients see an immediate
          savings opportunity; the rest are positioned to benefit over the course of their term.
          Your clients receive a monthly report either way, and Michael reaches out ahead of
          renewal when there is a genuine opportunity to act.
        </p>
        <p className="font-body text-[11px] text-gray-400 mt-3">
          Figures reflect Fox Mortgage&apos;s observed results and are not a guarantee of savings.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        {quickActions.map((action) => {
          // Disabled hero action (read-only demo): rendered as a non-interactive
          // card so planners see the affordance without a working write path.
          if (action.disabled) {
            return (
              <div
                key={action.title}
                aria-disabled="true"
                className="bg-lime/40 rounded-xl p-5 flex items-center gap-4 cursor-not-allowed select-none"
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <div className="font-body text-xs text-navy/75">Available in the live portal</div>
                </div>
              </div>
            )
          }
                ...
                return (
                  ...
                  >
                    <td className="px-5 py-3">
                      <Link
                        href={`/demo/fp/clients/${row.dealId}`}
