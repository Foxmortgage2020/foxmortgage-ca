...
const MD_COMPONENTS = {
  ...
  p: (p: any) => <p className="text-sm font-ui text-cool-700 leading-relaxed my-2" {...p} />,
  ul: (p: any) => <ul className="list-disc pl-5 my-2 space-y-1" {...p} />,
  ol: (p: any) => <ol className="list-decimal pl-5 my-2 space-y-1" {...p} />,
  li: (p: any) => <li className="text-sm font-ui text-cool-700 leading-relaxed" {...p} />,
  strong: (p: any) => <strong className="font-semibold text-navy" {...p} />,
  code: (p: any) => <code className="text-[12px] bg-cool-100 rounded px-1 py-0.5" {...p} />,
  a: (p: any) => <a className="text-navy underline hover:text-ink" target="_blank" rel="noreferrer" {...p} />,
  table: (p: any) => (
    <div className="overflow-x-auto my-3">
      <table className="w-full text-sm font-ui border border-cool-200" {...p} />
    </div>
  ),
  th: (p: any) => (
    <th className="text-left text-xs text-cool-500 uppercase tracking-wide border border-cool-200 bg-cool-50 px-2 py-1.5" {...p} />
  ),
  td: (p: any) => <td className="border border-cool-100 px-2 py-1.5 text-cool-700 align-top" {...p} />,
  blockquote: (p: any) => <blockquote className="border-l-2 border-cool-300 pl-3 my-2 text-cool-600" {...p} />,
  hr: () => <hr className="my-4 border-cool-200" />,
}
...
export default function LenderKnowledge({
  ...
  const approvedKnowledgeSection = (
    <div className="bg-white border border-cool-200 rounded-xl p-5">
      ...
      {approvedGroups.length === 0 ? (
        ...
        <div className="space-y-4">
          {approvedGroups.map(group => (
            <div key={group.topic}>
              <p className="text-xs font-semibold text-cool-500 uppercase tracking-wide mb-1.5">
              ...
              <div className="space-y-2">
                {group.claims.map(claim => {
                  ...
                  return (
                    <div key={claim.id} className="border border-cool-100 rounded-lg px-3 py-2">
                      <p className="text-sm font-ui text-cool-700">{claim.claimText}</p>
                      <div className="mt-1 flex flex-wrap items-center gap-2">
                        ...
                        {staleness && (
                          <span className="text-[11px] font-ui text-amber-700">{staleness}</span>
                        )}
                      </div>
                    </div>
                  )
      ...
      {pendingCount > 0 && (
        <p className="mt-3 pt-3 border-t border-cool-100 text-sm font-ui text-amber-800">
          {pendingCount} claim{pendingCount === 1 ? '' : 's'} awaiting approval{' '}
          <Link href="/portal/admin/approvals?tab=knowledge" className="font-semibold underline text-navy hover:text-ink">
            &rarr; Approvals
          </Link>
        </p>
      )}
      {docOpenError && <p className="mt-2 text-xs text-red-700 font-ui">{docOpenError}</p>}
    </div>
  )
  ...
  const sourceDocumentsSection = (
    <div className="bg-white border border-cool-200 rounded-xl p-5">
      ...
      {canUpload && (
        <div className="border border-dashed border-cool-300 rounded-lg p-4 mb-4">
          <div className="flex flex-wrap items-center gap-3">
            <input
              ...
              className="text-sm font-ui text-cool-600"
            />
            <select
              value={kind}
              onChange={e => setKind(e.target.value as KnowledgeUploadKind)}
              className="text-sm font-ui border border-cool-200 rounded-lg px-2 py-1.5 text-navy"
            >
            ...
            <button
              ...
              className="min-h-[40px] px-4 py-2 rounded-lg text-sm font-semibold font-ui bg-navy text-white hover:bg-navy-light disabled:opacity-50"
            >
          ...
          {uploadOutcome && <p className="mt-2 text-sm text-green-800 font-ui">{uploadOutcome}</p>}
          {uploadError && <p className="mt-2 text-sm text-red-700 font-ui">{uploadError}</p>}
        </div>
      )}
      {documents.length === 0 ? (
        <p className="text-sm text-cool-400 font-ui">No knowledge documents uploaded for this lender yet.</p>
      ) : (
          ...
          {documents.map(doc => {
            ...
            return (
              <div key={doc.id} className="py-2 flex flex-wrap items-baseline gap-x-3 gap-y-0.5">
                <span className="text-sm font-ui font-semibold text-navy break-words">{doc.docType}</span>
                {doc.knowledgeKind && (
                  <span className="text-[11px] text-cool-400 font-ui">{doc.knowledgeKind.replace(/_/g, ' ')}</span>
                )}
                <span className={`text-xs font-ui ${toneCls}`}>{status.text}</span>
              </div>
            )
          })}
        </div>
  ...
  if (loading) return <p className="text-sm text-cool-400 font-ui py-4">Loading lender knowledge…</p>
  if (error || !data) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {data.as_of && <span className="text-xs font-ui text-cool-500">profile as of {data.as_of}</span>}
        {stale && (
      ...
      <div className="bg-white border border-cool-200 rounded-xl p-5">
        ...
        {data.profile === null ? (
          ...
          <p className="text-sm text-cool-400 font-ui">The profile carries no figure entries yet.</p>
        ) : (
          <div className="divide-y divide-cool-50">
            {figures.map((f, i) => (
              <div key={i} className="py-1.5 flex flex-wrap items-baseline gap-x-3 gap-y-0.5">
                <span className="text-xs font-ui text-cool-500 capitalize">{f.path}</span>
                <span className="text-sm font-ui font-semibold text-navy break-words">{f.value}</span>
                <span className="text-[11px] text-cool-400 ml-auto">
        ...
        {gaps.length > 0 && (
          <div className="mt-3 pt-3 border-t border-cool-100">
            <p className="text-xs font-semibold text-cool-500 uppercase tracking-wide mb-1">Known gaps</p>
            <ul className="list-disc pl-5 space-y-0.5">
              {gaps.map((g, i) => (
                <li key={i} className="text-xs font-ui text-cool-500">
                  {g}
                </li>
              ))}
            </ul>
      ...
      <div className="bg-white border border-cool-200 rounded-xl p-5">
        <div className="flex items-center justify-between mb-3">
          ...
          <Link
            ...
            className="text-xs font-semibold text-navy hover:text-ink"
          >
