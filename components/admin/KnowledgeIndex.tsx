...
export default function KnowledgeIndex({ todayYMD }: { todayYMD: string }) {
  ...
  if (loading) return <p className="text-sm text-gray-400 font-body py-4">Loading the knowledge base…</p>
  if (error) {
    return (
      <div className="flex items-center justify-between gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
        <p className="text-sm text-amber-800 font-body">Knowledge base unavailable: {error}</p>
  ...
  if (lenders.length === 0) {
    return <p className="text-sm text-gray-400 font-body py-4">No lender knowledge files are bundled yet.</p>
  }
  return (
        ...
        return (
          ...
          >
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2">
              {l.as_of ? (
                <span className="text-xs font-body text-gray-500">profile as of {l.as_of}</span>
              ) : (
                <span className="text-xs font-body text-gray-500">machine profile withheld by design</span>
              )}
              {stale && (
