...
export default function RenewalDripQueue({ items, canDecide, demo }: {
  ...
  if (!items.length) {
    return <p className="text-sm text-cool-500 font-ui">No renewal messages waiting. Drafts land here as touches come due.</p>
  }
        ...
        return (
          <div key={it.touchId} className="rounded-lg border border-cool-200 bg-white p-4">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="font-semibold text-cool-800">{it.clientName ?? it.firstName ?? 'Client'}</span>
              <span className="rounded-full bg-cool-100 px-2 py-0.5 text-xs font-semibold text-cool-600">{TOUCH_LABEL[it.skeletonId] ?? it.skeletonId}</span>
              <span className="text-xs text-cool-500">matures {it.maturityDate}</span>
              {it.status === 'held' && (
            ...
            <p className="mt-2 text-xs text-cool-500 font-ui">Subject: <span className="font-semibold text-cool-700">{it.subject}</span></p>
            ...
            {it.sentences.length > 0 && (
              <div className="mt-2 text-xs text-cool-600 font-ui">
                <p className="font-semibold text-cool-700">Personalized content and its source:</p>
                {it.sentences.map((s, i) => (
                  <p key={i} className="mt-0.5">&ldquo;{s.text}&rdquo; <span className="text-cool-400">· from {s.source}</span></p>
                ))}
              </div>
            )}
            {it.sentences.length === 0 && (
              <p className="mt-2 text-xs text-cool-400 font-ui">No personalization on this draft (the skeleton floor; nothing grounded or the tier has no slot).</p>
            )}
            {it.dropped.length > 0 && (
              <p className="mt-1 text-[11px] text-cool-400 font-ui">{it.dropped.length} proposed sentence(s) refused by the fences (recorded in provenance).</p>
            )}
            ...
            {skipping === it.touchId && (
              <div className="mt-2 flex items-center gap-2">
                <input
                  ...
                  className="flex-1 rounded-md border border-cool-300 px-2 py-1.5 text-xs font-ui"
                />
                <button
                  type="button"
