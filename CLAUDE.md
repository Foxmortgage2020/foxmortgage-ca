...
- Small build, presentation only, over the B6.2 desk. No writes, no gate/authz change, no fox-underwriting change. The ONLY new read is `borrowers.relationship` (an already-granted table). Base `5d591a8`, 720 → **742 tests**.
...
- VERIFIED: tsc clean; `next build` green; **742 tests** (from 720: +16 doc-freshness, +9 desk B6.3, +demo). Lime audit green, ZERO additions (staleness is amber, advisory). RENDER PROOFS (dev-instance, ephemeral TEST admin created + DELETED, demo synthetic data only): desktop desk — Bank Statement "✓ Approved in Finmo" + amber "May be stale (uploaded 89 days ago)" sorted just below the flagged LOE, pills All 12 / Waiting 4 / Needs your look 5 / Done 3, sections "Jordan (parent)" + "Jordan (spouse)"; 375px single-column zero horizontal overflow. Demo exited, TEST admin deleted, server stopped.
- ADVERSARIAL REVIEW (2 dimensions, verified): ONE confirmed finding, fixed — the attribution reached the StateChip but not the request-EXPANSION's reviewed line, still a bare "Approved <date>" → now "Approved in Finmo <date>". Nothing else survived (no bare "Approved"/"by you" anywhere; the stale line renders beside the chip; no UI em dashes; zero lime).
