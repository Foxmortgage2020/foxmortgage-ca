...
### CRON_SECRET: what is known, and what is not

**I could not read the project environment this session.** The Vercel CLI token at
`~/Library/Application Support/com.vercel.cli/auth.json` returns
`403 {"invalidToken":true}` (the third recorded occurrence, after 2026-07-10 and
2026-07-17), and the Vercel MCP exposes no env-var tool. So the following is
INFERENCE, clearly labelled as such rather than reported as a check:

- `CRON_SECRET` appears nowhere in `.env.local`, nowhere in the code, and nowhere
  in this repo's docs except one ledger line describing the pattern as it exists
  on **fox-underwriting**, a different project.
- This project had no `vercel.json` and therefore has never had a Vercel cron, so
  nothing would have caused the variable to be created.

Conclusion, stated as the inference it is: `CRON_SECRET` is almost certainly
absent from the foxmortgage-ca project, and **it was not verified either way.**

That matters, because Vercel does not mint `CRON_SECRET` for you. With it unset,
the hourly invocation arrives carrying **no Authorization header at all** and this
route refuses it with a 401. That is the correct failure and it was chosen over
the alternatives on purpose: the route is publicly reachable (it must be, Clerk
would 401 the platform before the handler ran), and gating it on the undocumented
and unpromised `x-vercel-cron` header would have been a quieter but weaker gate.
A visible 401 every hour beats a route anyone can trigger.

**So the migration has one manual step left, and it is Michael's.** In the Vercel
dashboard, foxmortgage-ca → Settings → Environment Variables, add `CRON_SECRET`
with any long random value, scoped to **Production**, type Encrypted (not the
Vercel CLI, per the standing footgun: `vercel env add` silently writes
`type=sensitive`, which reads back empty). No redeploy is needed for the value to
be picked up by the next invocation. Until then the schedule fires hourly and is
refused, and `x-bridge-secret` remains the primary and only working path.
