...
## 2026-07-29, N-06: the lender notes card gains the CRM write

Base `b450711`. Continues N-05, which shipped the native generator in
fox-underwriting. fox-underwriting was READ ONLY this session, to confirm the
endpoint contract. The n8n workflow `dGtxpNedIDTJhwro` was not touched.

### The premise was wrong, and that is the main finding

The brief expected to find this repo pointing at the n8n Lender Notes Generator
webhook and to repoint it. **There was no caller to move.** Verified by grep at
the repo tip: `bef0669a` zero hits, `dGtxpNedIDTJhwro` zero hits,
`foxmortgage.app.n8n.cloud` two hits and neither is lender notes (the SMM enroll
base URL and a bookkeeping workflow deep link on the Revenue tab). The single
`lender-notes-generate` hit is `data-testid="lender-notes-generate"` in the card,
a test id and not a URL. Whatever triggered that workflow, it was never this
repo.

What the card actually does, and still does, is the OTHER lender notes path:
`LenderNotesCard` to `/api/portal/admin/gates/deals/[dealId]/lender-notes` to
`lib/gates.ts generateLenderNotes` to the Gates API `POST
/api/deals/{uuid}/lender-notes`, on a browser-minted Clerk token. That produces a
workbench DRAFT in `lender_notes` and sends nothing anywhere. It is a different
engine with a different output from the ported n8n generator, which writes
`Lender_Notes` onto the Zoho Deal. So the honest description of this session is
not a repoint. **The n8n workflow's job had no portal caller at all, and N-06
gives the N-05 engine its first one.** The draft path was left exactly as it was.

### Shipped

- `lib/lender-notes-bridge.ts`, the only module here that calls the engine, and
  server only. Kinds and a status map in the shape of the gates client. A
  refused bridge secret maps to 502 and never to 401: the caller's session was
  already gated on this side, so 401 would send an admin to sign in again over a
  secret they cannot fix by signing in.
- `POST /api/portal/admin/underwriting/lender-notes/[dealId]`, gated on the new
  admin-only `notes.crm.write`, refused in demo, `maxDuration` 300 because a
  generation is a model call over a whole file (18.4s observed).
  **The browser never names the Zoho record**: the card posts the workbench deal
  id it is already rendering and the route reads `zoho_potential_id` and
  `finmo_app_id` off the row through the read-only role. A client-supplied Zoho
  id on a CRM write is how one file's note lands on another file. Tested.
- Card: a separate "Write to the Zoho file" block under the draft, with Preview
  (the identical call with `dry_run`, so the preview is what a write would put on
  the file) and a timestamp-armed two-tap Write. DRAFT mode, the only mode any UI
  offers. Hidden entirely in demo. Reports each of the three writes by name, so a
  partial run reads as partial. A `skipped_recent` outcome offers an explicit
  forced press rather than retrying quietly, which matters because the engine
  returns `ok: true` on a skip and a caller reading `ok` alone would report a
  note that was never written.
- `notes.crm.write` is deliberately NOT `notes.generate`. That key's label is
  "draft only, nothing sent", and one key covering both would make the label a
  lie. Portal-local, not a gates key. Added to the admin-only list in
  tests/authority.test.ts with the reason recorded.

### Env: nothing for Michael to add, which was not the expected answer

The brief expected the bridge secret to be missing here. It is not.
`UW_BRIDGE_SECRET` in this repo is byte-identical to fox-underwriting's
`BRIDGE_SECRET` (sha256/12 `cb1e98d0f846` both sides, compared locally without
either value entering a transcript). The URL prefers an explicit
`UW_LENDER_NOTES_URL` and otherwise derives from `UW_BRIDGE_URL` by swapping
`/api/bridge/rooms` for `/api/bridge/lender-notes-generate`, so it reaches
wherever the room bridge already reaches. An unrecognised shape returns null
rather than a guess, and the card says the generator is not connected.
**CAVEAT, and it is the standing one:** the Vercel environment CANNOT be read
from here, so the above is proven for `.env.local` only. If the room sweep works
in production then both vars are there and this works too. `UW_LENDER_NOTES_URL`
is available as an override and is not required.

### Verified live, and what was not

- **Engine chain, real deal, dry run.** BRXM-F053107 (in progress, Zoho linked).
  Workbench row resolved through the read-only role, URL derived exactly as the
  module derives it, `x-bridge-secret` accepted: HTTP 200 in 18,435 ms.
  `outcome: generated`, `dryRun: true`, all three write flags FALSE, `errors: []`,
  model claude-opus-4-7, 8,673 in / 870 out, note 2,206 chars with DEAL SNAPSHOT
  present, no surviving em or en dash, correct sign-off, audit
  `a655884e-19a0-4ef7-99ba-286816610f5e`. The note body was never printed to a
  transcript or a file. Engine notes carried two honest gaps worth knowing:
  Graph email context is off, and one call transcript was capped at 6,000 chars.
- **The route refuses a signed-out caller.** Live against the dev server: POST
  dry run, POST real, and GET all 401 before the handler, since the path is
  deliberately not in middleware `publicRoutes` (unlike the sweep, which needs a
  machine door).
- **Route logic**, 16 tests: the gate key, both refusals, demo, the
  ignore-the-client's-Zoho-id property, mode defaulting and the 422 on an unknown
  mode, and a partial write staying visible through a failure.
- **NOT proven end to end through a signed-in browser session.** Doing that means
  driving the card on a live deal room, and the standing UI-test discipline
  forbids pointer events on pages listing live records, with the armed Write
  control sitting beside Preview. The unproven link is Clerk session to
  `apiPermission`, which is the same mechanism as every other admin route here.
  The first press is Michael's by design anyway.

### Open, for the chat lane

The first REAL `Lender_Notes` write is Michael's press, DRAFT mode, on a file he
picks. It was not done here. Retiring `dGtxpNedIDTJhwro` follows that press, not
this session. Left active and untouched.
...
## 2026-07-28, documentation sync: today's decisions into the docs
